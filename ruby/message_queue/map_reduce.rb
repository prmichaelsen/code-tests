...
def calculate_stats(values)
  ...
  # Initialize accumulators for even and odd
  even_sum = 0
  even_count = 0
  odd_sum = 0
  odd_count = 0
  
  # Subscribe to even and odd events to accumulate stats
  q.subscribe "even" do |value|
    even_sum += value
    even_count += 1
  end
  
  q.subscribe "odd" do |value|
    odd_sum += value
    odd_count += 1
  end
  
  # Map: Partition values into even/odd and publish
  values.each do |value|
    type = value % 2 == 0 ? "even" : "odd"
    q.publish type, value
  end
  
  # Reduce: Calculate averages
  even_avg = even_count > 0 ? even_sum.to_f / even_count : 0
  odd_avg = odd_count > 0 ? odd_sum.to_f / odd_count : 0
  
  {
    :even => { :sum => even_sum, :count => even_count, :average => even_avg },
    :odd => { :sum => odd_sum, :count => odd_count, :average => odd_avg }
  }
end
